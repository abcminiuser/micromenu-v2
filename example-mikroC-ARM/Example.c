...
{
    if(Button(&GPIOB_IDR, 0, DEBOUNCE_MS, 0))
        return BUTTON_PARENT;
    else if(Button(&GPIOB_IDR, 1, DEBOUNCE_MS, 0))
        return BUTTON_PREVIOUS;
    else if(Button(&GPIOB_IDR, 2, DEBOUNCE_MS, 0))
        return BUTTON_NEXT;
    else if(Button(&GPIOB_IDR, 3, DEBOUNCE_MS, 0))
        return BUTTON_CHILD;
    else if(Button(&GPIOB_IDR, 4, DEBOUNCE_MS, 0))
        return BUTTON_MINUS;
    else if(Button(&GPIOB_IDR, 5, DEBOUNCE_MS, 0))
        return BUTTON_PLUS;
    return BUTTON_NONE;
}
...
MENU_ITEM (Menu_3,  Menu_1,    Menu_2,    NULL_MENU, Menu_31,   NULL,       NULL,      NULL,         NULL,      "Menu 3          ");
 DATA_ITEM(Menu_31, Menu_32,   Menu_33,   Menu_3,    NULL_MENU, NULL,       NULL,      NULL,         NULL,      "Edit bit        ", BIT_TYPE,      GPIOA_ODR, 5);
 DATA_ITEM(Menu_32, Menu_33,   Menu_31,   Menu_3,    NULL_MENU, NULL,       NULL,      NULL,         NULL,      "Edit signed     ", SIGNED_TYPE,   GPIOC_ODR, 1);
 DATA_ITEM(Menu_33, Menu_31,   Menu_32,   Menu_3,    NULL_MENU, NULL,       NULL,      NULL,         NULL,      "Edit unsign     ", UNSIGNED_TYPE, GPIOC_ODR, 1);
#endif
// clang-format on
...
/*
// Pin direction
sbit LCD_RS_Direction at GPIOB_ODR.B10;
...
int main(void)
{
    GPIO_Digital_Output(&GPIOA_BASE, _GPIO_PINMASK_5); // Set PORTA.B5 pin as output

    GPIO_Digital_Output(&GPIOC_BASE, _GPIO_PINMASK_ALL); // Set PORTC pins as outputs

    GPIO_Digital_Input(&GPIOB_BASE, _GPIO_PINMASK_LOW); // Set LOW PORTB pins as inputs
